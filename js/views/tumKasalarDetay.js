  ...
  const rows = dagilim.map(({ kasaId, ad, emoji, gelen, cikan, net }) => {
    ...
    return `
      <div class="tkd-dagilim-satir" data-kasa-id="${kasaId}">
        ...
        <span style="font-size:15px;font-weight:700;color:${netRenk}">${net >= 0 ? '+' : ''}${kisaltilmisRakam(net)}</span>
      </div>`;
  }).join('');
...
function buildListe(kasalar, baslangic, bitis, islemler, kategoriler, cariler) {
  ...
  const rows = hareketler.map(islem => {
    ...
    if (islem.tip === 'transfer') {
      ...
      tutarStr  = `↔ ${formatTL(islem.tutar)}`;
      tutarRenk = 'var(--accent)';
      ...
      tutarStr  = `+${formatTL(islem.tutar)}`;
      tutarRenk = 'var(--success)';
    } else {
      icon      = kategori?.emoji || '▼';
      title     = islem.aciklama || kategori?.ad || 'Gider';
      tutarStr  = `-${formatTL(islem.tutar)}`;
      tutarRenk = 'var(--danger)';
    }
